...
def position_possible(i1, j1, pv, id, couleur):
    ...
    if i1 - pv < -1:
        ...
        while i < pv :
            ...
            if jeu_soi[i1-i][j1] != 0:
                valide_haut = 0
            i = i + 1
        # la saisie en haut est possible
        if valide_haut == 1 :
            l = CASE
            h = pv * CASE
            x = j1*CASE
    ...
    if j1 + pv - 1 >= N:    
        ...
        while i < pv :
            ...
            if jeu_soi[i1][j1+i] != 0:
                valide_droite = 0
            i = i + 1
        # la saisie à droite est possible
        if valide_droite == 1 :
            l = pv * CASE
            h = CASE
            x = j1*CASE
    ...
    # 4) affichage de l'aide à la saisie à gauche
    if j1 - pv <= -2 :
        valide_gauche = 0
    else:
        # test des cases vides à gauche
        i = 0
        while i < pv :
            # case non vide
            if jeu_soi[i1][j1-i] != 0:
                valide_gauche = 0
            i = i + 1
        # la saisie à gauche est possible
        if valide_gauche == 1 :
            l = pv*CASE
            h = CASE
            x = (j1-pv+1)*CASE
...
def saisie_bateau_soi(pv,id):
    ...
    i1 = clic1[1]//CASE
    j1 = clic1[0]//CASE
    # - il faut au moins une direction valide
    while position_possible(i1, j1, pv, id, gris) != 1:
        clic1 = wait_clic()
        i1 = clic1[1]//CASE
        j1 = clic1[0]//CASE
        # - la case cliquée est vide
